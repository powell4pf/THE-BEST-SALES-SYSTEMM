...
public sealed class DeliveryNotesController : ControllerBase
{
    ...
    public DeliveryNotesController(IDeliveryNoteService service, ICurrentUserService currentUser) { _service = service; _currentUser = currentUser; }
    ...
    [HttpPost, Permission("deliverynotes.manage")] public async Task<ActionResult<Guid>> Create(CreateDeliveryNoteRequest request, CancellationToken cancellationToken) { var id = await _service.CreateAsync(request, _currentUser.UserId, cancellationToken); return CreatedAtAction(nameof(GetById), new { id }, id); }
    ...
    [HttpDelete("{id:guid}"), Permission("deliverynotes.manage")] public async Task<IActionResult> Delete(Guid id, CancellationToken cancellationToken) => await _service.DeleteAsync(id, _currentUser.UserId, cancellationToken) ? NoContent() : NotFound();
}
