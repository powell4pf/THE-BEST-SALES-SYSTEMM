...
public sealed class CreditNotesController : ControllerBase
{
    ...
    [HttpDelete("{id:guid}"), Permission("creditnotes.delete")] public async Task<IActionResult> Delete(Guid id, CancellationToken cancellationToken) => await _service.DeleteAsync(id, _currentUser.UserId, cancellationToken) ? NoContent() : NotFound();
}
