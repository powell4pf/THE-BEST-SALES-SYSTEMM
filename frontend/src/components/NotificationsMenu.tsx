...
export function NotificationsMenu() {
  ...
  return <div ref={containerRef} className="relative">
    ...
    {open && <div className="absolute right-0 top-12 z-50 w-[min(24rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl dark:border-white/10 dark:bg-slate-900">
      <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-white/10"><div><h2 className="text-sm font-semibold text-slate-950 dark:text-white">Notifications</h2><p className="text-xs text-slate-500 dark:text-slate-400">Document activity and generated records</p></div><Button variant="ghost" size="sm" className="h-8 px-2 text-xs" onClick={() => markAllRead.mutate()} disabled={!unreadCount || markAllRead.isPending}><CheckCheck className="h-3.5 w-3.5" />Read all</Button></div>
      <div className="max-h-[22rem] overflow-y-auto p-2">{notifications.length === 0 ? <div className="px-4 py-8 text-center text-sm text-slate-500 dark:text-slate-400"><Bell className="mx-auto mb-2 h-6 w-6 opacity-50" />No document notifications yet.</div> : notifications.map((notification) => <button type="button" key={notification.id} className={`flex w-full gap-3 rounded-xl p-3 text-left transition hover:bg-slate-100 dark:hover:bg-white/10 ${notification.readAt ? '' : 'bg-sky-50 dark:bg-sky-500/10'}`} onClick={() => openNotification(notification.id, notification.route, notification.readAt)}><span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-slate-950 text-white dark:bg-white dark:text-slate-950"><FileText className="h-4 w-4" /></span><span className="min-w-0 flex-1"><span className="flex items-center justify-between gap-2"><span className="text-sm font-semibold text-slate-950 dark:text-white">{notification.title}</span>{!notification.readAt && <span className="h-2 w-2 shrink-0 rounded-full bg-sky-500" />}</span><span className="mt-0.5 block text-xs text-slate-600 dark:text-slate-300">{notification.message}</span><span className="mt-1 block text-[11px] text-slate-400">{relativeTime(notification.createdAt)}</span></span></button>)}</div>
    </div>}
  </div>;
}
