    ...
    <div className="app-bg flex min-h-screen items-center justify-center px-4 py-10">
      <div className="grid w-full max-w-6xl gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <Card className="flex flex-col justify-center p-8">
          ...
          {mode === 'login' && online ? <div className="relative my-6">
          ...
          {mode === 'login' && online ? <div className="w-full">
            <GoogleLogin onSuccess={handleGoogleSuccess} onError={handleGoogleError} />
          </div> : null}

          {mode === 'login' && !online ? <p className="mt-4 text-center text-xs text-amber-700 dark:text-amber-300">Offline access is available only for a password account that previously signed in successfully on this device. Cached data and new drafts will synchronize when the system reconnects.</p> : null}
